...
def create_review(tmp_path: Path, **overrides):
    ...
    review = create_retained_compatibility_review(summary, manifest, **values)
...
def test_acceptance_is_named_hashed_and_non_retroactive(tmp_path):
    ...
    verify_retained_compatibility_review(
        review, summary, manifest, artifact_directory=artifact_directory,
    )
    ...
    tightened = next(item for item in review["scenario_evidence"] if item["scenario"] == "tightened")
...
def test_follow_up_decisions_require_actions_and_acceptance_rejects_them(tmp_path):
    ...
    common = {
        "artifact_directory": artifact_directory,
        "reviewer_name": "Alex Reviewer", "reviewer_role": "Data Platform Owner",
        "review_ticket": "GOV-140",
        "rationale": (
    ...
    with pytest.raises(IntervalPolicyRetainedCompatibilityReviewError, match="at least one"):
        create_retained_compatibility_review(
            summary, manifest, decision="request_historical_annotation", **common,
        )
    with pytest.raises(IntervalPolicyRetainedCompatibilityReviewError, match="cannot contain requested_actions"):
        create_retained_compatibility_review(
            summary, manifest, decision="accept_non_retroactive_transition",
            requested_actions=["Prepare a separate annotation proposal for historical rows."], **common,
        )
    review = create_retained_compatibility_review(
        summary, manifest, decision="request_compatibility_reassessment",
        requested_actions=["Repeat the comparison when one more complete trend run is retained."], **common,
    )
...
def test_review_rejects_early_timestamp_source_tampering_and_self_tampering(tmp_path):
    summary, manifest, artifact_directory, _ = source_evidence(tmp_path)
    with pytest.raises(IntervalPolicyRetainedCompatibilityReviewError, match="cannot precede"):
        create_retained_compatibility_review(
            summary, manifest, artifact_directory=artifact_directory,
            decision="accept_non_retroactive_transition", reviewer_name="Alex Reviewer",
            reviewer_role="Data Platform Owner", review_ticket="GOV-140",
    ...
    with pytest.raises(IntervalPolicyRetainedCompatibilityReviewError, match="summary digest"):
        verify_retained_compatibility_review(
            review, changed_summary, manifest, artifact_directory=artifact_directory,
        )
    review["rationale"] += " changed"
    with pytest.raises(IntervalPolicyRetainedCompatibilityReviewError, match="review hash"):
        verify_retained_compatibility_review(
            review, summary, manifest, artifact_directory=artifact_directory,
        )
...
def test_review_satisfies_versioned_schema(tmp_path):
    ...
    schema = json.loads((ROOT / "data-contracts" /
                         "interval_policy_retained_compatibility_review_schema.json").read_text(encoding="utf-8"))
    assert list(Draft202012Validator(schema, format_checker=FormatChecker()).iter_errors(review)) == []
...
def test_writer_and_cli_are_immutable(tmp_path):
    ...
    outputs = write_retained_compatibility_review(
        direct, review, summary, manifest, artifact_directory=artifact_directory,
    )
    ...
    output = tmp_path / "cli"
    args = [
        "--compatibility-summary", str(paths["summary"]),
        "--compatibility-manifest", str(manifest_path),
        "--artifact-directory", str(artifact_directory),
        "--decision", "accept_non_retroactive_transition",
        "--reviewer-name", "Alex Reviewer", "--reviewer-role", "Data Platform Owner",
        "--review-ticket", "GOV-140", "--rationale",
        "Accept the non-retroactive transition while retaining historical evidence.",
        "--reviewed-at-utc", REVIEW_TIMESTAMP, "--review-id", REVIEW_ID,
