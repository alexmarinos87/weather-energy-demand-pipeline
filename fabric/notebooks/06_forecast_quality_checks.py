...
# Validates the newest baseline run and records blocking evidence.
...
def run_checks(spark_session):
    ...
    checks = [
        ...
        },
        {
            ...
            "failed_rows": int(
                spark_session.table(METRICS_TABLE)
                .where(F.col("run_id") == run_id)
                .limit(1)
                .count()
                == 0
        ...
        },
        {
            ...
            "failed_rows": spark_session.table(PREDICTIONS_TABLE)
            .where(F.col("run_id") == run_id)
            .where(
        ...
        },
        {
            ...
            "failed_rows": spark_session.table(PREDICTIONS_TABLE)
            .where(F.col("run_id") == run_id)
            .where(F.col("event_timestamp_utc") <= F.col("trained_through_utc"))
            .count(),
        },
        {
            "check_name": "forecast_metrics_valid",
            "failed_rows": spark_session.table(METRICS_TABLE)
            .where(F.col("run_id") == run_id)
            .where(
                (F.col("observation_count") <= 0)
                | (F.col("mae_mw") < 0)
                | (F.col("rmse_mw") < 0)
                | (F.col("evaluation_start_utc") <= F.col("trained_through_utc"))
